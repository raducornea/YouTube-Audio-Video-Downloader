...
def combine_audio(video_name, audio_name, out_name):
    ...
    clip = moviepy.editor.VideoFileClip(video_name).subclip()
...
def downloader_mp3():
    ...
    title = audio.title
    current_audio_path = downloads_path
    possible_existing_mp3 = current_audio_path + '\\' + title + '.mp3'

    if os.path.isfile(possible_existing_mp3):
...
def downloader_mp4():
    ...
    current_video_path = downloads_path
...
if __name__ == '__main__':
    ...
    Button(window, text='DOWNLOAD MP3', font='arial 15 bold', bg='yellow', pady=20, command=downloader_mp3).place(x=50,
                                                                                                                  y=130)
    Button(window, text='DOWNLOAD MP4', font='arial 15 bold', bg='yellow', pady=20, command=downloader_mp4).place(x=270,
                                                                                                                  y=130)

    # padx = <---> of the button
    # pady = similar, but for height
